...
export default function Onboarding() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await completeOnboardingAsync(user.email, {
      dni: dni.trim(),
      phone: phone.trim(),
    ...
    <div className="min-h-screen bg-slate-100 text-slate-900 font-sans antialiased flex items-center justify-center p-4">
      <div className="w-full max-w-lg">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 shadow-sm">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-4">
              <div className="text-sm font-semibold text-slate-700">Contacto de Emergencia</div>
            ...
            <button
              type="submit"
              className="w-full py-3.5 bg-blue-600 hover:bg-blue-700 text-white font-semibold text-sm rounded-full shadow-lg shadow-blue-600/20 hover:shadow-blue-600/30 hover:-translate-y-0.5 active:translate-y-0 transition-all duration-150 cursor-pointer flex items-center justify-center gap-2"
            >
        ...
        <p className="text-center text-xs text-slate-500 mt-6">
          Tus datos se utilizaran para emergencias y logistica del equipo.
        </p>
      </div>
    </div>
  );
